...
<?php
/////////////////////////////////////////////////this sction is for adding category
...
/////////////////////////////////////this the section for enabling category
	if($_POST["categoryvalue"]== "3")
{
	...
	$categoryid = $_POST['pcategory'];
		...
		if($categoryid == "0"){
			...
			echo"SELECT THE CATEGORY TO ENABLE";
			exit();
		}
	
	/////////////////////////////////////////////////////////////////////////////////this section enables all the product under the category
	$sql_enable_category = $con->query("select * from product where product_cat = '$categoryid'");
		
	if($sql_enable_category->num_rows > 0)
	{
		while($row_enable_category = mysqli_fetch_array($sql_enable_category))
		{
			$product_id = $row_enable_category['id'];
			
			enableproduct($product_id);
		}
	}
	
	//////////////////////////////////this section calls the function that enables all the brand under this category
	$sql_enablebrand_forcategory = $con->query("select * from brands where category = '$categoryid'");
		
	if($sql_enablebrand_forcategory->num_rows > 0)
	{
		while($row_enablebrand_forcategory = mysqli_fetch_array($sql_enablebrand_forcategory))
		{
			$brand_id = $row_enablebrand_forcategory['brand_id'];
...
if($_POST["categoryvalue"]== "4")
{
	...
	//////////////////////////////////this section calls the function that disables all the brand under this category
	$sql_check_brand = $con->query("select * from brands where category = '$categoryid'");
		
	if($sql_check_brand->num_rows > 0)
	{
		while($row_brand = mysqli_fetch_array($sql_check_brand))
		{
...
function disableclass($id)
{
		...
		$disable_class = $con->query("update class set status = '0' where sn = '$id'");
		if(!$disable_class)
		{
