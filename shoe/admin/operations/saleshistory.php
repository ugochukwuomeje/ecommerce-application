			...
			?>
				...
				<div class='col-md-9'>
					<?php
						...
						$sql_confirm_order_product = "SELECT * FROM confirm_order_product WHERE status = 1";
					...
					";
							while($row_query_confirm_order_product =  mysqli_fetch_array($sql_query_confirm_order_product)){
								...
								$date = $row_query_confirm_order_product['date'];
								$id = $row_query_confirm_order_product['date'];
								
								echo"<tr><td><img src='$image' style='width:20%; height:20%' class='img-responsive'></td><td>$name</td><td>$price</td><td>$quantity</td><td>$total_price</td><td>$date</td><td><a class='btn btn-success btn-small' role='button' href='shippingdetails.php?email=$email'>view buyer details</a></td></tr>";						
							}
							...
							echo"<div class='alert alert-danger' role='alert'><center>NO RECORD IN THE DATABASE</center></div>";
						}
					echo"</table>";
					?>
				</div>	
